   ...
   <div id="sub_wrapper">
	...
	<table  id="border_table" border="1" align="left" >
		<form method="get" action="salaryDays.do" name="salaryDays" onSubmit="return checkIt()" >
			<tr><input type="text" id="salary_days" style="border: 1px solid;" name="salary_days" onclick="fnPopUpCalendar(salary_days,salary_days,'yyyy - mm - dd')"> 일정별 조회<input type="text" style="border: 1px solid;" id="salary_day" name="salary_day" onclick="fnPopUpCalendar(salary_day,salary_day,'yyyy - mm - dd')"></tr>
			<tr><input type="submit" class="button" value="조회하기">
			<input type="button" class="button" value="전체보기" onclick="window.open('/gw4/salary/salaryList.do','_self')">
	</tr>
		</form>
	</table>
	...
	<form>
		...
		<tr>
			...
			<td align="center">
			<input type="button" class="buttonS" value="수정" onclick="location='salaryUpdate.do?salary_code=${salary.salary_code }'"/>
			<input type="button" class="buttonS" value="삭제" onclick="salaryDelete(${salary.salary_code})"/>
			</td>
		</tr>
		</c:forEach>
